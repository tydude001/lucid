...
lucid runs locally, but two of its servers can edit your projects and run an
agent, so anything that lets someone else reach them is in scope:

- **`lucid web`** binds loopback, checks the `Host` header, and requires
...
- **`lucid mcp --transport http`** binds loopback and checks `Host`, the same
  way. A way to reach it from off the machine without `--allow-remote` is a
  vulnerability.
- **`lucid review serve`** is meant to be reached from other devices and
