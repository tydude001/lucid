...
- **The MCP SDK is v2. `FastMCP` no longer exists** — it is `MCPServer`, from
  `mcp.server`. Training priors overwhelmingly say `FastMCP`; check the
  installed package before writing server code, not your memory of the API.
- **`rtk`'s pytest filter reports "No tests collected" for suites that ran.**
  Re-run through `rtk proxy uv run pytest` before believing a surprising
  result.
