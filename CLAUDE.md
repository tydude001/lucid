...
- **The README screenshots are regenerated by `scripts/capture_screenshots.py`,
  never captured by hand.** Three rounds of drift, one cause each time: the
  theme seed, the viewport and the demo project's state were re-derived from
  memory and one of them got forgotten. The route had been written down in
  prose beside the harness since the second round and the third session still
  did not follow it — prose beside a harness is not a mechanism. Three of the
  script's steps are assertions, because each is something a round got wrong
  *silently*: the theme is read back off `data-theme` rather than trusted to
  the `localStorage` write, the Edit shot refuses unless the agent pane really
  is showing "Export complete" (a reload destroys it, and the caption claims
  it), and the set refuses a mean-luma spread over 30. **A shot is sized to
  its own content**, which is not the same as measuring every descendant: the
  stretching containers all end at the pane's bottom edge, so skip anything
  that does, and never read a scroll container's `scrollHeight` for it —
  under-full, that reports the box. HISTORY.md § The screenshots stopped
