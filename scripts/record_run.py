...
def record(work: Path, model: str | None, port: int, cdp_port: int, keep: bool) -> int:
    ...
    try:
        ...
        brief = agent_trial.DEMO_BRIEF.format(media=_tilde(media), project=_tilde(project), output=_tilde(output))
        stamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S")
        run_dir = work / "runs" / stamp
        run_dir.mkdir(parents=True)
        (run_dir / "brief.txt").write_text(brief, encoding="utf-8")
        (run_dir / "scoring.json").write_text(
            json.dumps({"phrases": agent_trial.DEMO_PHRASES, "min_clips": agent_trial.DEMO_MIN_CLIPS,
